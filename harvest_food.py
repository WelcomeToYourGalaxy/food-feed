#!/usr/bin/env python3
"""
harvest_food.py — the food wire: who owns the food system, who funds what is
said about it, what it does to the people eating it and the land it runs on,
and what the label actually certifies.

After "The Food and Drink Industries", which the section places under
Suppression by "Representation" rather than under control of resources,
because these companies sit between a body and its food rather than upstream
of it.

What it covers: ownership and concentration of the food system; ultra-processed
food and the conditions that track it; nutrition science bought and the
narrative it produces; the health and nutrition bodies taking industry money;
front groups and manufactured doubt; lobbying against bills, labels and taxes;
executives seated on the boards that decide; who the marketing is aimed at;
what a lawsuit costs against what the practice earns; sugar and drink taxes;
and then the organic question the section takes apart at length — residues
found in certified produce, certification resting on paperwork, one certifying
agent per several hundred operations, a five per cent annual testing
requirement, and a certifier paid out of the crop it approves. Then livestock
and climate, the scale of slaughter, food waste, the bans on lab-grown meat,
the narrowness of what is actually grown, water, dependency, and the
alternatives.

A recipe or a restaurant review is not this subject. Every subject term carries
the words it must appear beside.

    python3 harvest_food.py
    python3 harvest_food.py --dry-run
"""
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; food-feed/1.0; "
...
# THE SUBJECTS
#
# Who owns the food system, who funds what is said about it, what it does to
# the people eating it and the land it runs on, and what the label certifies.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so a recipe or a restaurant review cannot reach the wire.
# --------------------------------------------------------------------------
TOPICS = [
    ("ownership", "Who owns the food system", [
        ("food system", ["own*", "control*", "concentration", "consolidat*", "power", "map"]),
        ("food giant", []), ("food conglomerate", []), ("agribusiness", ["consolidat*", "power", "merger", "control*", "profit"]),
        ("four firm", []), ("market concentration", ["food", "grocery", "seed", "meat", "dairy", "retail"]),
        ("merger", ["food", "beverage", "grocery", "meat", "seed", "agri*"]),
        ("acquisition", ["food", "beverage", "grocery", "brand", "agri*"]),
    ...
    ("ultraprocessed", "Ultra-processed food and what it does", [
        ("ultra-processed", []), ("ultraprocessed", []), ("nova classification", []),
        ("processed food", ["health", "linked", "study", "risk", "consumption", "disease"]),
        ("obesity", ["food", "drink", "sugar", "diet", "rate", "industry", "policy"]),
        ("diabetes", ["food", "drink", "sugar", "diet", "rate", "industry"]),
        ("heart disease", ["diet", "food", "sugar", "fat", "salt", "linked"]),
        ("dementia", ["diet", "food", "linked", "processed"]),
        ("high fructose corn syrup", []), ("trans fat", []),
        ("salt", ["reduction", "target", "content", "reformulation", "policy"]),
        ("sugar content", []), ("reformulation", ["food", "drink", "salt", "sugar"]),
    ]),
    ("science", "Funded nutrition science", [
        ("nutrition science", ["funded", "industry", "sponsored", "influence", "conflict"]),
        ("industry-funded", ["study", "research", "trial", "review"]),
        ("funded by", ["coca-cola", "nestle", "pepsico", "the industry", "food industry", "sugar industry"]),
        ("sponsored study", []), ("sponsored research", []),
        ("conflict of interest", ["nutrition", "dietitian", "study", "author", "panel", "guideline"]),
        ("dietary guideline", ["influence", "industry", "committee", "conflict", "revised"]),
        ("undisclosed funding", []), ("manufactured doubt", []), ("merchants of doubt", []),
        ("retracted", ["study", "paper", "nutrition", "trial"]),
    ]),
    ("organisations", "Funded health and nutrition bodies", [
    ...
    ("frontgroups", "Front groups and doubt", [
        ...
        ("public relations", ["campaign", "industry", "food", "beverage", "firm hired"]),
        ("denial", ["industry", "science", "health", "campaign"]),
        ("internal documents", ["industry", "food", "sugar", "tobacco", "revealed"]),
    ]),
    ("lobbying", "Altering the bills", [
        ("food lobby*", []), ("beverage lobby*", []), ("sugar lobby*", []), ("meat lobby*", []),
        ("lobbying", ["food", "beverage", "sugar", "meat", "dairy", "agri*", "nutrition"]),
        ("bill", ["watered", "blocked", "amended", "industry", "opposed", "shelved"]),
        ("regulation", ["blocked", "delayed", "watered", "opposed", "weakened", "food", "labelling"]),
        ("advertising restriction", ["opposed", "delayed", "watered", "dropped"]),
        ("marketing to children", ["restrict*", "ban", "code", "rules"]),
        ("revolving door", ["food", "agri*", "regulator", "agency"]),
    ]),
    ("boards", "Executives inside the institutions", [
        ("advisory board", ["industry", "executive", "appointed", "conflict"]),
        ("advisory committee", ["industry", "executive", "appointed", "conflict"]),
        ("board member", ["executive", "industry", "conflict", "appointed"]),
        ("appointed", ["former executive", "industry figure", "lobbyist"]),
        ("food stamp", []), ("snap", ["soda", "junk", "restriction", "benefits", "purchase"]),
        ("public procurement", ["food", "school meal", "hospital", "standard"]),
        ("school meal", ["standard", "contract", "supplier", "nutrition", "cut"]),
    ]),
    ("targeting", "Who the marketing is aimed at", [
        ("targeted advertising", ["children", "communities", "low-income", "minority", "teens"]),
        ("marketing", ["children", "targeted", "low-income", "minority", "aggressive", "school"]),
        ("advertising spend*", ["food", "drink", "snack", "soda", "fast food"]),
        ("sponsorship", ["sport", "stadium", "team", "youth", "event"]),
        ("influencer", ["food", "drink", "snack", "marketing", "undisclosed"]),
        ("junk food", ["advert*", "marketing", "ban", "watershed", "children"]),
    ...
    ("taxes", "Taxes on sugar and drink", [
        ...
        ("levy", ["sugar", "drink", "soft drink", "beverage"]),
        ("front-of-pack", []), ("warning label", ["food", "drink", "sugar", "high in"]),
        ("nutri-score", []), ("traffic light label", []),
        ("tax repeal", ["soda", "sugar", "drink"]),
    ]),
    ("organic", "What the organic label certifies", [
        ("organic certification", []), ("organic label", []), ("certifying agent", []),
        ("certifier", ["organic", "conflict", "income", "paid by", "audit"]),
        ("decertif*", []), ("certification revoked", []), ("lost certification", []),
        ("organic fraud", []), ("organic standard", ["weakened", "review", "breach", "enforcement"]),
    ...
    ("residues", "Pesticide residues and testing", [
        ...
        ("maximum residue level", []), ("glyphosate", ["residue", "food", "detected", "limit", "ruling"]),
        ("pesticide", ["detected", "residue", "food", "banned", "limit", "sample"]),
        ("contaminant", ["food", "detected", "sample", "limit"]),
        ("heavy metal", ["food", "baby food", "detected", "limit"]),
        ("testing programme", ["residue", "food", "pesticide"]),
        ("testing program", ["residue", "food", "pesticide"]),
    ]),
    ("imports", "Imports, ports and traceability", [
    ...
    ("livestock", "Livestock and the climate", [
        ("livestock", ["emission*", "climate", "methane", "greenhouse", "footprint"]),
        ("methane", ["livestock", "cattle", "farming", "dairy", "reduction"]),
        ("meat industry", ["emission*", "climate", "footprint", "deforestation"]),
        ("dairy", ["emission*", "climate", "methane", "footprint"]),
        ("deforestation", ["cattle", "soy", "beef", "feed", "supply chain"]),
        ("feed", ["soy", "imported", "deforestation", "livestock"]),
        ("emissions", ["food system", "agriculture", "livestock", "diet"]),
        ("footprint", ["diet", "food", "meat", "dairy", "consumption"]),
    ]),
    ("slaughter", "The scale of it, and the conditions", [
        ("slaughterhouse", []), ("abattoir", []), ("slaughter", ["number", "billion", "million", "conditions", "practice"]),
        ("animal welfare", ["law", "standard", "breach", "footage", "inspection", "ruling"]),
        ("factory farm*", []), ("intensive farming", []), ("caged", ["hens", "pigs", "ban", "phase out"]),
        ("undercover footage", []), ("ag-gag", []),
        ("stunning", ["failure", "law", "practice", "requirement"]),
        ("live export", []), ("culled", ["birds", "pigs", "herd", "flock", "million"]),
    ]),
    ("waste", "What is thrown away", [
        ("food waste", []), ("food loss", []), ("wasted", ["food", "produce", "harvest", "per cent"]),
        ("surplus", ["food", "produce", "redistribut*", "destroyed"]),
        ("best before", []), ("use by", ["date", "label", "confusion"]),
        ("landfill", ["food", "waste", "organic"]),
        ("food bank", ["surplus", "donation", "demand", "redistribut*"]),
    ]),
    ("labgrown", "Alternatives and the bans on them", [
        ("lab-grown meat", []), ("cultivated meat", []), ("cultured meat", []),
        ("cell-cultured", []), ("precision fermentation", []),
        ("plant-based", ["ban", "label", "restrict*", "naming", "law", "market"]),
        ("meat label*", ["ban", "restrict*", "law", "naming"]),
        ("veggie burger", []), ("alternative protein", []),
        ("ban", ["cultivated meat", "lab-grown", "plant-based label", "cell-cultured"]),
    ]),
    ("diversity", "The narrowness of what is grown", [
        ...
        ("plant species", ["consumption", "edible", "used", "cultivated"]),
        ("crop*", ["provide", "share of calories", "per cent of", "dependence", "narrow", "diversity"]),
        ("calories", ["share", "per cent", "plant-sourced", "staple", "three crops"]),
    ]),
    ("water", "Drink, and the water behind it", [
        ("bottled water", []), ("water extraction", ["bottling", "company", "permit", "drought"]),
        ("water use", ["agriculture", "food", "beverage", "irrigation", "intensive"]),
        ("aquifer", ["depleted", "extraction", "bottling", "irrigation"]),
        ("drinking water", ["contaminat*", "unsafe", "access", "supply", "nitrate"]),
        ("soft drink", ["water", "extraction", "plant", "permit"]),
        ("irrigation", ["depletion", "conflict", "over-extraction", "rights"]),
    ]),
    ("dependency", "Dependency and self-sufficiency", [
        ("food security", ["policy", "risk", "threatened", "national", "strategy"]),
...
ANCHOR = [
    ...
    # subject; who owns it, who funds what is said about it, what it does to
    # people and land, and what the label actually certifies, is.
    "food system", "food industry", "agribusiness", "ultra-processed",
    "ultraprocessed", "nutrition science", "dietary guideline", "food lobby",
    "beverage lobby", "sugar tax", "soda tax", "front-of-pack",
    "organic certification", "certifying agent", "pesticide residue",
    "maximum residue level", "food fraud", "traceability", "food waste",
    "lab-grown meat", "cultivated meat", "alternative protein",
    "animal welfare", "factory farming", "slaughterhouse", "live export",
    "livestock emissions", "food security", "food sovereignty",
    "marketing to children", "junk food advertising", "infant formula marketing",
    "front group", "industry-funded", "sponsored study", "conflict of interest",
...
BLOCK = [
    # the food pages, which are most food coverage and none of this subject
    "recipe", "how to cook", "cooking tips", "meal plan", "meal prep",
    "restaurant review", "best restaurants", "michelin star", "chef's table",
    "taste test", "food trend", "menu launch", "new menu", "pop-up",
    "cocktail", "wine pairing", "brewery tour", "bake off", "food festival",
    "top 10 dishes", "where to eat", "brunch spot", "street food guide",
    "diet plan", "weight loss tips", "what i eat in a day",
...
DECIDED = [
    ...
    "fined", "settled", "recalled", "suspended", "withdrawn", "decertified",
    "convicted", "ordered", "seized", "blocked", "phased out",
]
INSTITUTIONAL = [
    "world health organization", "world health organisation", "fao", "unicef",
    "efsa", "food standards agency", "food and drug administration", "usda",
    "codex alimentarius", "european commission", "european food safety",
    "ministry of agriculture", "ministry of health", "auditor general",
    "national audit office", "government accountability office", "inspector general",
    ...
    "randomised trial", "randomized trial", "cohort study",
]
MEASURED = [
    ...
    "per capita", "grams", "kilogram", "tonnes", "samples", "prevalence",
]
PENDING = [
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Food agencies, regulators & UN bodies"},
            {"id": "research", "label": "Research & food system monitoring"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Food, health & animal organisations"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_food.json not written")
        return payload
